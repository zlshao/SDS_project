...
- SDS_SK_type: *flowkey*. *flowkey* is the key extracted from each arriving packet. The *flowkey* is used as input to the hash function, and the value output by the hash function is used to locate the buckets that should be updated in SDS. All other options except 11 and 12 contain *Protocol* by default.
- SDS_SK_feature: The SDS bucket composition. The bucket composition selected for detecting slow port scans is "abhj23". In practice, the bucket composition can be changed to accomplish different measurement tasks as needed.
